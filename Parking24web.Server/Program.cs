...
builder.Services.AddCors(options =>
{
    ...
    options.AddPolicy("AllowReactApp", policy =>
    {
        policy.WithOrigins("https://localhost:5173", "http://localhost:5173",
                          "https://0.0.0.0:5173", "http://0.0.0.0:5173") // 외부 접근용 추가
              .AllowAnyMethod()
              .AllowAnyHeader()
              .AllowCredentials(); // SignalR에 필수
    });
});
...
{
    Console.WriteLine($"설정 로딩 중 오류: {ex.Message}");
}
...
else
{
    ...
    app.UseHsts();
    // 운영환경에서는 모든 origin 허용
    app.UseCors("AllowAll");
}

// 정적 파일 설정 (React 빌드 파일들)
...
appLogger.LogInformation($"=== PLC 및 주차 시스템 시작 ===");
appLogger.LogInformation($"사이트: {siteName}");
...
// 사이트 설정 클래스들 (PLCHub.cs에 함께 정의 되어있음)
